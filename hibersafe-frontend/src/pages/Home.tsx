...
export default function Home() {
  ...
  const calculateResultsB = async () => {
    ...
    if (!stacktrace){
      ...
      setLoading(true);
      try{
        let returnInfoB = await axios.post<any, HibersafeAPI>(`http://${window.location.hostname}:8080/api/question/exceptionEnum/${exception}`, {stacktrace});
  ...
  useEffect(() => {
    if (toLog && estrategia === "A") {
      axios.post<any, any>(`http://${window.location.hostname}:8080/api/log/`, {estrategia, id, dados: resultsA, stacktrace, exception})
    }
    setToLog(false);
  ...
  useEffect(() => {
    if (toLog && estrategia === "B") {
      axios.post<any, any>(`http://${window.location.hostname}:8080/api/log/`, {estrategia, id, dados: resultsB, stacktrace, exception})
    }
    setToLog(false);
